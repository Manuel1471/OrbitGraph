...
| `@orbitgraph/vue` | Vue 3 component with all runtime events, reactive inputs, cleanup, and full instance access. |
| `@orbitgraph/svelte` | Svelte 5 action/controller with all runtime callbacks, reactive inputs, cleanup, and full instance access. |
| `@orbitgraph/headless` | Stateful Node.js/SSR editing, history, routes, diff, analytics, layouts, JSON, SVG and PDF without DOM. |
...
Collapsed clusters render a summary node with member count and internal/external relationship metrics. External relationships are consolidated by source, target, and type while the original graph remains unchanged.

```ts
graph.enableClusterLevelOfDetail();
graph.enableClusterLevelOfDetail(120); // optional camera-distance threshold
graph.isClusterLevelOfDetailEnabled();
graph.disableClusterLevelOfDetail();
```

At 5,000+ nodes OrbitGraph starts with aggregate type groups, split into chunks of at most 1,000 members. Untyped graphs use deterministic segments. Zooming in expands only the cluster nearest the camera target; zooming out returns to summaries. `disableClusterLevelOfDetail()` explicitly mounts full detail and should be used carefully on massive data.
...
`CanvasGraphRenderer` draws active nodes and batches relationships into one Canvas path for low-power embedding. `MapLibreGraphLayer` writes nodes with longitude/latitude metadata to a MapLibre GeoJSON source. `GraphPluginRegistry` runs application-installed plugins and makes their data sources and style rules available at runtime. SVG is vector output; PDF is loaded on demand and returns a promise.

`renderMode: "canvas"` does not construct `WebGLRenderer` or request a WebGL context. WebGL uses `InstancedMesh` for nodes; frustum virtualization compacts the active instance count so off-camera nodes are omitted from the draw call.

### Worker and CRDT runtime

```ts
const result = await graph.computeInWorker("radial", abortController.signal);
const session = graph.connectYjs(provider);
session.setPresence({ name: "Ada", color: "#22d3ee", cursor: { x: 10, y: 20 } });
const operations = [{ type: "add-node", node: { id: "new" } }] as const;
graph.applyOperations(operations);
session.applyLocalOperations(operations);
```

Worker payloads use transferable typed arrays for positions, edge indices, weights, type indices and cluster assignments. `GraphYjsProvider` is transport-neutral: WebSocket, WebRTC and persisted Yjs providers can forward document and awareness updates.

### Headless / SSR

```ts
import { HeadlessOrbitGraph } from "@orbitgraph/headless";
const serverGraph = new HeadlessOrbitGraph(data);
serverGraph.applyOperations([{ type: "add-node", node: { id: "new" } }]);
const analytics = serverGraph.analyze();
const positions = serverGraph.layout("radial");
const svg = serverGraph.exportSVG();
const pdf = await serverGraph.exportPDF({ title: "Graph report" });
```

`HeadlessOrbitGraph` also provides `undo()`, `redo()`, `selectNodes()`, `compare()`, `findWeightedPath()`, `findKShortestPaths()`, validated operations, and JSON export. Stateless helper functions remain available for one-shot jobs.

```ts
const svg = graph.exportSVG();
graph.downloadSVG("graph.svg");
const pdf = await graph.exportPDF({ title: "Architecture report" });
await graph.downloadPDF("report.pdf", { title: "Architecture report" });
```
...
Supported layouts are `"force"`, `"radial"`, `"grid"`, `"hierarchical"`, `"dag"`, `"sankey"`, `"timeline"`, `"bipartite"`, `"geographic"`, `"concentric"`, `"sphere"`, and `"arc"`. DAG uses Dagre to rank dependencies and reduce crossings. Sankey uses d3-sankey with widths proportional to `weight`. Timeline reads `layoutOptions.timeField` (default `"time"`), bipartite uses `bipartiteTypes`, and geographic reads longitude/latitude metadata fields.

The active renderer and physics consolidate only parallel links with equal endpoints and type. Their `data` includes `aggregateCount`, `aggregateWeight`, and defined `aggregatedLinkIds`; analytics, filters and exports retain every original link.
...
Camera fits animate smoothly and pointer, wheel, touch, or WASD input cancels an in-progress fit immediately.
...
const vectorSVG = graph.exportSVG();
const reportPDF = await graph.exportPDF({ title: "Graph report" });
```
...
- Leave automatic aggregate LOD enabled for 5,000+ nodes; semantic zoom materializes one nearby cluster at a time.
- Frustum culling hides already-created offscreen objects. It reduces drawing but does not remove existing GPU allocations.
...
- In Graph Scale Lab, Worker progress covers data generation; renderer construction and camera framing are reported as separate stages and may be cancelled before handoff.
