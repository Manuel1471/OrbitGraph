...
## v1.5 analytics and themes

```ts
import { calculateCoreNumbers, detectDegreeAnomalies, findConnectedComponents, graphThemes } from "@orbitgraph/core";

const components = findConnectedComponents(data);
const core = calculateCoreNumbers(data);
const anomalies = detectDegreeAnomalies(data);
const theme = graphThemes.midnight;
```

`calculateNodeSimilarity()` compares neighborhoods with Jaccard similarity. `aggregateClusters()` creates summary cluster nodes and consolidated inter-cluster links for collapsed views.
...
For multiuser synchronization, `GraphYjsCollaboration` stores graph operations in a Yjs CRDT and exposes Yjs Awareness presence, cursors, and selections. Attach any provider implementing `GraphYjsProvider` (WebSocket, WebRTC, or an application transport); concurrent updates converge without last-write-wins replacement.
...
- [`@orbitgraph/vue`](https://www.npmjs.com/package/@orbitgraph/vue): Vue 3 bindings.
- [`@orbitgraph/svelte`](https://www.npmjs.com/package/@orbitgraph/svelte): Svelte action bindings.
- [`@orbitgraph/headless`](https://www.npmjs.com/package/@orbitgraph/headless): DOM-free analytics, layouts, SVG, and PDF.
